            ...
            for tag in tm.tags:
                with gr.Row(elem_id=f"{tag.name}_row_container"):
                    ...
                    with gr.Column(elem_id=f"{tag.name}_description_row_container", scale=3):
                        desc_textbox = gr.Textbox(label='Description', value=tag.description, elem_id=f'{tag.name}_desc_textbox')
                        file_rows.append(desc_textbox)
                    with gr.Column(elem_id=f"{tag.name}_models_row_container", scale=3):
                        models_textbox = gr.Textbox(label='Models', value=tag.models_to_string(), elem_id=f'{tag.name}_models_textbox')
